...
palavras = ["Qual a integral de 5X??"]
md = randrange(0,len(palavras))
z = palavras[md]
...
def  reiniciarJogo():
    global pontos, velocidade, colidiu, escolha, ganhou, md, z, vida, vidas, m, M, l, verifica
    md = randrange(0,len(palavras))
    z = palavras[md]
    vida = "<3<3<3"
    l = m[md]
...
class Professor(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image = sprite_sheet_boss.subsurface((0,3 * 83), (73,83))
        self.image = pygame.transform.scale(self.image, (73*1.5,83*1.5))
        self.rect = self.image.get_rect()
        self.rect.center = (x + 450,y)
...
while True:
    ...
    if ganhou == True:
        ...
        palavra_secreta = exibe_mensagem(z, 15, preto)
    ...
    nivel_aparece = exibe_mensagem("NIVEL 1", 30, preto)
